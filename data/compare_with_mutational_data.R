...
ann.S$class <- dis
sel.traits <- c("gender","ethnicity")#,"class","vital_status","X12p.Status","X5q.Status")
props <- getProportions(medecom.set,K=7,lambda=0.001)
...
p.vals <- apply(props,1,function(x){
  sapply(sel.traits,function(trait){
    ...
    x <- x[!na.trait]
	cat(paste("Comparing",levels(trait)),":",length(unique(trait)),"\n")
    if(length(unique(trait))>2){
      p.val <- kruskal.test(x,g=trait)$p.value
    }else if(length(unique(trait))==2){
      p.val <- t.test(x[as.character(trait)==levels(trait)[1]],x[as.character(trait)==levels(trait)[2]])$p.value
    }else{
      p.val <- NA
    }
  })
